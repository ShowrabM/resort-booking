  ...
  widgets.forEach(widget => {
    ...
    bookingForm.innerHTML = `
      ...
      <div class="rbw-form-grid">
        ...
        <label>মোবাইল নম্বর*<input type="tel" name="rbw_phone" required></label>
        <label>অতিথি সংখ্যা*<input type="number" min="1" value="1" name="rbw_guests" required></label>
        <label>NID / ID (ছবি আপলোড করুন)*<input type="file" accept="image/*" capture="environment" name="rbw_nid" required></label>
      </div>
    ...
    bookingForm.querySelector('.rbw-submit').addEventListener('click', async () => {
      ...
      if(!name || !phone || !guests || !nidFile){
        showAlert('err','সব তথ্য পূরণ করুন এবং NID আপলোড করুন');
        return;
      }
